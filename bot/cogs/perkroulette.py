...
class Roulette(Cog):
    ...
    def _CreateGspread(self):
        ...
        data['private_key_id'] = os.getenv("G_API_KEY_ID").replace("'", '"')
        print(data['private_key_id'])
        data['private_key'] = os.getenv("G_API_KEY").replace("'", '"')
        print(data['private_key'])
        data['client_email'] = os.getenv("G_API_MAIL").replace("'", '"')
        print(data['client_email'])
        data['client_id'] = os.getenv("G_API_C_ID").replace("'", '"')
        print(data['client_id'])
        data['client_x509_cert_url'] = os.getenv("G_API_CURL").replace("'", '"')
